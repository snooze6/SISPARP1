...
void main_alltoall(int argc, char **argv){
    ...
    while(j<=4194304) {
        ...
        char *msg1 = (char *) malloc(sizeof(char) * j * size), *msg2 = (char *) malloc(sizeof(char) * j * size);
        ...
        for (k = 0; k < 2; k++) {
            ...
            for(i=0;i<n_sample;i++){
                MPI_Alltoall(msg1, j, MPI_BYTE, msg2, j, MPI_BYTE, MPI_COMM_WORLD);
            }
...
//#ifndef INCLUDE_MAIN
//#define INCLUDE_MAIN
//void mainasdf(int argc, char **argv) {main_alltoall(argc, argv);}
//#endif