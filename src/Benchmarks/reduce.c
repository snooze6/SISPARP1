...
void main_reduce(int argc, char** argv){
    ...
    while(j<=4194304) {
        ...
        for (k = 0; k < 2; k++) {
            ...
            for(i=0;i<n_sample;i++){
                MPI_Reduce(msg1, msg2, j/sizeof(MPI_FLOAT), MPI_FLOAT, MPI_SUM, i%size, MPI_COMM_WORLD);
            }
...
//#ifndef INCLUDE_MAIN
//#define INCLUDE_MAIN
//void main(int argc, char **argv) {main_reduce(argc, argv);}
//#endif