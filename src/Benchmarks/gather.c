...
void main_gather(int argc, char** argv){
    ...
    while(j<=4194304) {
        ...
        char *msg1 = (char *) malloc(sizeof(char) * j), *msg2 = (char *) malloc(sizeof(char) * j * size);
        ...
        for (k = 0; k < 2; k++) {
            ...
            for(i=0;i<n_sample;i++){
                MPI_Gather(msg1, j, MPI_BYTE, msg2, j,MPI_BYTE,i % size,MPI_COMM_WORLD);
            }
