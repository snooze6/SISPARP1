...
void main_scatter(int argc, char** argv){
    ...
    while(j<=4194304) {
        ...
        char *msg1 = (char *) malloc(sizeof(char) * j * size), *msg2 = (char *) malloc(sizeof(char) * j);
        ...
        for (k = 0; k < 2; k++) {
            ...
            for(i=0;i<n_sample;i++){
                MPI_Scatter(msg1,j,MPI_BYTE, msg2, j, MPI_BYTE, i % size, MPI_COMM_WORLD);
            }
...
//#ifndef INCLUDE_MAIN
//#define INCLUDE_MAIN
//void main(int argc, char **argv) {main_gather(argc, argv);}
//#endif