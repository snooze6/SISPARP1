...
//Compilar mpicc SendRecv.c -lm (para la librería math.h)
main(int argc, char** argv){
	...
	while(X<=OVERALL_VOL){
		...
		for(i=0; i<n_sample;i++){
			...
			MPI_Sendrecv(k, X, MPI_BYTE, (R+P+1)%P, 0, k, X, MPI_BYTE, (R+P-1)%P, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}

		//El tiempo final sería el tiempo en este instante menos el tiempo inicial
		tiempoFinal = (MPI_Wtime() - tiempoInicial)/(n_sample*2);
		
		//Imprimo los datos, solo en un proceso
		if(R==0) printf("%d\t\t%d\t\t%f\t%f\n", X, n_sample, (tiempoFinal*1000000), (X/(tiempoFinal)/(1024*1024)));
