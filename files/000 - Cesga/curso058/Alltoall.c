...
//Compilar mpicc Alltoall.c -lm (para la librería math.h)
main(int argc, char** argv){
	...
	char *k; //Buffer de recepcion
	char *k2; //Buffer de envio
	...
	while(X<=OVERALL_VOL){
		...
		for(i=0; i<n_sample;i++){
				MPI_Alltoall(k, X, MPI_BYTE, k2, X, MPI_BYTE, MPI_COMM_WORLD);
		}
