...
double ejecuta_benchmark (int tamano_division, int numero_mensajes) {
    ...
    mensaje_envia = (char *) malloc(numero_procesos * tamano_division * sizeof(char));
    if (mensaje_envia == NULL) {
        printf("No se ha podido hacer el malloc de mensaje_envia\n");
        return -1;
    }
    mensaje_recibe = (char *) malloc(tamano_division * sizeof(char));
    if (!mensaje_recibe) {
        printf("No se ha podido hacer el malloc e mensaje_recibe\n");
        return -1;
    }
    ...
    if (process_number == 0) {
        for (i = 0; i < (numero_procesos * tamano_division); i++) {
            *(mensaje_envia + i) = 'a';
        }
    }
    ...
    for (i = 0; i < numero_mensajes; i++) {
        ...
        MPI_Scatter(mensaje_envia, tamano_division, MPI_BYTE,
            mensaje_recibe, tamano_division, MPI_BYTE, 0, MPI_COMM_WORLD);
    }
...
void imprime_resultados_en_fichero (int numero_procesos, long int * tamano_mensaje, long int * numero_mensajes, double * results, double * _time) {
    int i;
    FILE * results_file = fopen("results/results_scatter.csv", "a");
