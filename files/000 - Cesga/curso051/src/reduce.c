...
int main(int argc, char *argv[])
{
	...
	int rank=0,size;
	double inicio,total;
	int i,actual=0,X=sizeof(MPI_FLOAT),n_barr;
	int n_sample = MSGSPERSAMPLE;
	void  * msg = malloc(X);
	void * rcvMsg = malloc(sizeof(MPI_FLOAT));
	...
	for(i=0;i<N_WARMUP;i++){
		MPI_Reduce(msg,rcvMsg,X/sizeof(MPI_FLOAT),MPI_FLOAT,MPI_SUM, i % size,MPI_COMM_WORLD);
	}
