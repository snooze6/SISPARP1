...
void scatter_benchmark(unsigned char * const send_recevice_buf ,
                       int const size,
                       int n)
{
    while (n--) {
        MPI_Scatter(MPI_IN_PLACE, size, MPI_BYTE, send_recevice_buf, size, MPI_BYTE, 0, MPI_COMM_WORLD) ;
#if VERBOSE
        sleep(1) ;
...
void scatter_benchmark_root(unsigned char * const send_recevice_buf ,
                            int const size,
                            int n)
{
    while (n--) {
        MPI_Scatter(send_recevice_buf, size, MPI_BYTE, MPI_IN_PLACE, size, MPI_BYTE, 0, MPI_COMM_WORLD) ;
#if VERBOSE
        sleep(1) ;
        printf("Sended %d bytes.\n", SIZE) ;
#endif
...
void benchmark(int world_rank,
    ...
    *start = MPI_Wtime () ;
    ...
    /* SCATTER */
    if (!world_rank) {
        scatter_benchmark_root(recv_buf_bytes, *current_size, *n_sample) ;
    } else {
        scatter_benchmark(recv_buf_bytes, *current_size, *n_sample) ;
    }
